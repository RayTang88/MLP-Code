...
class Net:
    def __init__(self):
        ...
        self.w1 = tf.Variable(tf.zeros(shape=[784,512], dtype=tf.float32),name='weights')
        self.b1 = tf.Variable(tf.truncated_normal(shape=[100, 512], stddev=tf.sqrt(1 / 256), dtype=tf.float32), name='weights')
        # self.b1 = tf.Variable(tf.zeros(512),dtype=tf.float32,name='bias')
        self.w2 = tf.Variable(tf.zeros(shape=[512, 256], dtype=tf.float32))
        self.b2 = tf.Variable(tf.truncated_normal(shape=[100, 256], stddev=tf.sqrt(1 / 256), dtype=tf.float32))
        # self.b2 = tf.Variable(tf.zeros(256), dtype=tf.float32)
        self.w3 = tf.Variable(tf.zeros(shape=[256, 128], dtype=tf.float32))
        self.b3 = tf.Variable(tf.truncated_normal(shape=[100, 128], stddev=tf.sqrt(1 / 128), dtype=tf.float32))
        # self.b3 = tf.Variable(tf.zeros(128), dtype=tf.float32)
        self.wo = tf.Variable(tf.zeros(shape=[128, 10], dtype=tf.float32))
        self.bo = tf.Variable(tf.truncated_normal(shape=[100, 10], stddev=tf.sqrt(1 / 64), dtype=tf.float32))
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        writer.close()
        if not os.path.exists('./mympl'):
            os.makedirs('./mympl')
        saver = tf.train.Saver()
        saver.save(sess, './mympl/mympl_data.ckpt')
